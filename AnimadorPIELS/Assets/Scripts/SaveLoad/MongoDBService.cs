...
public class MongoDBService
{
    ...
    private const string AnimationsPath = "animations";
    ...
    public async Task<bool> SaveAnimation(string animationName, List<KeyframeData> keyframes)
    {
        if (!isConnected)
        {
            Debug.LogError("Pose API: Not connected. Can't save animation.");
            return false;
        }

        try
        {
            AnimationSaveRequest payload = new AnimationSaveRequest
            {
            ...
            string json = JsonUtility.ToJson(payload);
            var url = BuildUrl($"{AnimationsPath}/{UnityWebRequest.EscapeURL(animationName)}");
            using UnityWebRequest request = new UnityWebRequest(url, UnityWebRequest.kHttpVerbPUT);
            request.uploadHandler = new UploadHandlerRaw(System.Text.Encoding.UTF8.GetBytes(json));
            request.downloadHandler = new DownloadHandlerBuffer();
            request.SetRequestHeader("Content-Type", "application/json");
            ...
            if (request.result != UnityWebRequest.Result.Success)
            {
                Debug.LogError($"Pose API: Failed to save animation: {request.error}");
                return false;
            }
        ...
        {
            Debug.LogError($"Pose API: Failed to save animation: {ex.Message}");
            return false;
        }
    }

    public async Task<List<KeyframeData>> LoadAnimation(string animationName)
    {
        if (!isConnected)
        {
            Debug.LogError("Pose API: Not connected. Can't load animation.");
            return null;
        }
        ...
        try
        {
            ...
            {
                if (request.responseCode == 404)
                    Debug.LogWarning($"Pose API: Animation '{animationName}' not found");
                else
                    Debug.LogError($"Pose API: Failed to load animation: {request.error}");
                return null;
            }

            AnimationResponse response = JsonUtility.FromJson<AnimationResponse>(request.downloadHandler.text);
            if (response == null || response.keyframes == null)
            {
                Debug.LogError($"Pose API: Invalid response while loading animation '{animationName}'");
                return null;
            }

            return response.keyframes;
        }
        catch (Exception ex)
        {
            Debug.LogError($"Pose API: Failed to load animation: {ex.Message}");
            return null;
        }
    }

    public async Task<List<string>> GetAllAnimationNames()
    {
        if (!isConnected)
        {
            Debug.LogError("Pose API: Not connected. Can't retrieve animation names.");
            return new List<string>();
        }

        try
        {
            var url = BuildUrl(AnimationsPath);
            using UnityWebRequest request = UnityWebRequest.Get(url);
            ApplyCommonRequestOptions(request);
            await SendRequest(request);
            ...
            if (request.result != UnityWebRequest.Result.Success)
            {
                Debug.LogError($"Pose API: Failed to retrieve animation names: {request.error}");
                return new List<string>();
            }

            AnimationNamesResponse response = JsonUtility.FromJson<AnimationNamesResponse>(request.downloadHandler.text);
            return response?.animationNames ?? new List<string>();
        }
        catch (Exception ex)
        {
            Debug.LogError($"Pose API: Failed to retrieve animation names: {ex.Message}");
            return new List<string>();
        }
    }

    public async Task<bool> DeleteAnimation(string animationName)
    {
        if (!isConnected)
        {
            Debug.LogError("Pose API: Not connected. Can't delete animation.");
            return false;
        }
        ...
        try
        {
            ...
            if (request.responseCode == 404)
            {
                Debug.LogWarning($"Pose API: Animation '{animationName}' not found for deletion");
                return false;
            }

            Debug.LogError($"Pose API: Failed to delete animation: {request.error}");
            return false;
        }
        catch (Exception ex)
        {
            Debug.LogError($"Pose API: Failed to delete animation: {ex.Message}");
            return false;
        }
    }
