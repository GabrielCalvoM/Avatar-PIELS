...
public class SaveLoadPose : MonoBehaviour
{
    ...
    [SerializeField] SkinnedMeshRenderer avatarFace;
    ...
    public PoseData CaptureCurrentPose()
    {
        PoseData pose = CapturePose(avatarSpine, includeFacialExpression: false);
        pose.facialExpression = CaptureBlendshapesDirect();
        return pose;
    }

    public void ApplyCurrentPose(PoseData pose)
    {
        ApplyPose(avatarSpine, pose, applyFacialExpression: false);
        ApplyBlendshapesDirect(pose.facialExpression);
    }

    private FacialExpressionData CaptureBlendshapesDirect()
    {
        FacialExpressionData data = new FacialExpressionData();
        if (avatarFace == null || avatarFace.sharedMesh == null) return data;

        int count = avatarFace.sharedMesh.blendShapeCount;
        for (int i = 0; i < count; i++)
            data.blendshapes.Add(new BlendshapeWeightData
            {
                name   = avatarFace.sharedMesh.GetBlendShapeName(i),
                weight = avatarFace.GetBlendShapeWeight(i)
            });

        return data;
    }

    private void ApplyBlendshapesDirect(FacialExpressionData data)
    {
        if (data == null || avatarFace == null || avatarFace.sharedMesh == null) return;

        foreach (var bs in data.blendshapes)
        {
            int idx = avatarFace.sharedMesh.GetBlendShapeIndex(bs.name);
            if (idx >= 0) avatarFace.SetBlendShapeWeight(idx, bs.weight);
        }
    }

    public PoseData LerpPose(PoseData a, PoseData b, float t)
    {
        PoseData result = new PoseData();

        Dictionary<string, BoneData> bMap = new();
        foreach (var bone in b.bones) bMap[bone.name] = bone;

        foreach (var boneA in a.bones)
        {
            if (!bMap.TryGetValue(boneA.name, out BoneData boneB))
            {
                result.bones.Add(boneA);
                continue;
            }
            result.bones.Add(new BoneData
            {
                name        = boneA.name,
                localPosition = Vector3.Lerp(boneA.localPosition, boneB.localPosition, t),
                localRotation = Quaternion.Slerp(boneA.localRotation, boneB.localRotation, t),
                localScale    = Vector3.Lerp(boneA.localScale, boneB.localScale, t)
            });
        }

        if (a.facialExpression != null && b.facialExpression != null)
        {
            Dictionary<string, float> bWeights = new();
            foreach (var bs in b.facialExpression.blendshapes) bWeights[bs.name] = bs.weight;

            FacialExpressionData face = new FacialExpressionData
            {
                leftEyelid   = Mathf.Lerp(a.facialExpression.leftEyelid,   b.facialExpression.leftEyelid,   t),
                rightEyelid  = Mathf.Lerp(a.facialExpression.rightEyelid,  b.facialExpression.rightEyelid,  t),
                raiseEyebrow = Mathf.Lerp(a.facialExpression.raiseEyebrow, b.facialExpression.raiseEyebrow, t),
                angleEyebrow = Mathf.Lerp(a.facialExpression.angleEyebrow, b.facialExpression.angleEyebrow, t),
                mouthH       = Mathf.Lerp(a.facialExpression.mouthH,       b.facialExpression.mouthH,       t),
                mouthV       = Mathf.Lerp(a.facialExpression.mouthV,       b.facialExpression.mouthV,       t),
            };
            foreach (var bs in a.facialExpression.blendshapes)
            {
                float weightB = bWeights.TryGetValue(bs.name, out float w) ? w : bs.weight;
                face.blendshapes.Add(new BlendshapeWeightData { name = bs.name, weight = Mathf.Lerp(bs.weight, weightB, t) });
            }
            result.facialExpression = face;
        }
        else
        {
            result.facialExpression = a.facialExpression ?? new FacialExpressionData();
        }
