...
const userAnimationsCollectionName = process.env.MONGODB_USER_ANIMATIONS_COLLECTION || "user_animations";
...
function registerAnimationRoutes(routeBase, collection) {
  app.get(routeBase, async (req, res) => {
    try {
      const docs = await collection
        .find({}, { projection: { _id: 0, animationName: 1 } })
        .sort({ animationName: 1 })
        .toArray();
      res.json({ animationNames: docs.map((doc) => doc.animationName) });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  });

  app.get(`${routeBase}/:animationName`, async (req, res) => {
    try {
      const animationName = normalizePoseName(req.params.animationName);
      if (!animationName) {
        res.status(400).json({ error: "animationName is required" });
        return;
      }
      const doc = await collection.findOne({ animationName });
      if (!doc) {
        res.status(404).json({ error: "Animation not found" });
        return;
      }
      res.json({ animationName: doc.animationName, keyframes: doc.keyframes || [] });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  });

  app.put(`${routeBase}/:animationName`, async (req, res) => {
    try {
      const animationNameFromPath = normalizePoseName(req.params.animationName);
      const animationNameFromBody = normalizePoseName(req.body && req.body.animationName);
      const animationName = animationNameFromBody || animationNameFromPath;
      ...
      const keyframes = req.body && req.body.keyframes;
      if (!Array.isArray(keyframes)) {
        res.status(400).json({ error: "keyframes array is required" });
        return;
      }

      const now = new Date();
      const existing = await collection.findOne({ animationName });

      if (existing) {
        await collection.updateOne(
          { animationName },
          { $set: { keyframes, updatedAt: now } }
        );
      } else {
        await collection.insertOne({
          animationName,
  ...
  app.delete(`${routeBase}/:animationName`, async (req, res) => {
    try {
      const animationName = normalizePoseName(req.params.animationName);
      if (!animationName) {
        res.status(400).json({ error: "animationName is required" });
      ...
      const result = await collection.deleteOne({ animationName });
      if (result.deletedCount === 0) {
        res.status(404).json({ error: "Animation not found" });
        return;
      }
...
async function start() {
  ...
  userAnimationsCollection = db.collection(userAnimationsCollectionName);
  ...
  await userAnimationsCollection.createIndex({ animationName: 1 }, { unique: true });
  ...
  registerAnimationRoutes("/api/animations", userAnimationsCollection);
